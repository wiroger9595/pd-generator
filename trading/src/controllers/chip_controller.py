...
async def chip_buy_us(
    ...
    send_fundamental_report("美股籌碼", "buy", items)
    logger.info(f"[Chip] 美股買進完成，推薦 {len(items)} 檔")
    return result
...
async def chip_sell_us(
    ...
    send_fundamental_report("美股籌碼", "sell", items)
    logger.info(f"[Chip] 美股賣出完成，訊號 {len(items)} 檔")
    return result
...
async def chip_buy_tw(
    ...
    items = result.get("recommendations", [])
    send_fundamental_report("台股籌碼", "buy", items)
    logger.info(f"[Chip] 台股買進完成，推薦 {len(items)} 檔")
    return result
...
async def chip_sell_tw(
    ...
    items = result.get("sell_signals", [])
    send_fundamental_report("台股籌碼", "sell", items)
    logger.info(f"[Chip] 台股賣出完成，訊號 {len(items)} 檔")
    return result
